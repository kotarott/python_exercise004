...
class Order:
    ...
    def get_oturi(self, payment):
        self.sum()
        oturi = payment - self.item_sum
        if oturi < 0:
            print("不足金額:{}".format(-1 * oturi))
        else:
            print("お釣り:{}".format(oturi))
...
def main():
    ...
    payment = input("支払金額を入力してください。\n>>> ")
    order.get_oturi(int(payment))
...
def add_item(cls):
    ...
    cls.add_item_order(input_code, int(input_num))
